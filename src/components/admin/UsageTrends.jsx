...
export default function UsageTrends() {
    ...
    useEffect(() => {
        const getStats = async () => {
            try {
                ...
                setBookingsChart(data && data.bookingsPieChart && data.bookingsPieChart.length > 0 
                ...
                setIssuesChart(data && data.issuesPieChart && data.issuesPieChart.length > 0 
                    ? data.issuesPieChart 
                    : [
                        { name: 'pool', value: 100 }
                    ]);
                    
                setTotalBookings(data && data.totalBookings !== undefined ? data.totalBookings : 3);
                setTotalIssues(data && data.totalIssues !== undefined ? data.totalIssues : 1);
    ...
    const prepareDataForExport = (logs) => {
        return logs.map(log => {
            ...
            if (log.timestamp && log.timestamp.seconds) {
                formattedLog.formattedTimestamp = new Date(log.timestamp.seconds * 1000).toLocaleString();
            } else {
    ...
    const exportPDF = (logs) => {
        ...
        // Add summary section
        doc.setFontSize(12);
        doc.text(`Total Bookings: ${totalBookings || 0} (${bookingsChange || 0} from last month)`, 14, 25);
        doc.text(`Total Issues: ${totalIssues || 0} (${issuesChange || 0} from last month)`, 14, 32);
        ...
        captureChartForPDF(bookingsChartElement, 'Proportion of Bookings by Facility', () => {
            captureChartForPDF(issuesChartElement, 'Proportion of Issues by Facility', () => {
    ...
    const exportChartsAsPDF = () => {
        ...
        // Add summary data
        doc.setFontSize(12);
        doc.text(`Total Bookings: ${totalBookings} (${bookingsChange} from last month)`, 14, 25);
        doc.text(`Total Issues: ${totalIssues} (${issuesChange} from last month)`, 14, 32);
        ...
        if (bookingsChart && issuesChart) {
            ...
            const charts = [
                { element: bookingsChart, title: 'Proportion of Bookings by Facility', y: 40 },
                { element: issuesChart, title: 'Proportion of Issue Reports by Facility', y: 150 }
            ];
            ...
            <section className="main-section">
                ...
                <section className="right">
                    ...
                    <section className="Buttons">
                        <button onClick={() => exportCSV(filteredLogs)} className="export-btn csv-btn">Export as CSV</button>
                        <button onClick={() => exportPDF(filteredLogs)} className="export-btn pdf-btn">Export Complete Report</button>
                    </section>
                        ...
                        <table className="log-table">
                            ...
                            <tbody>
                                ...
                                filteredLogs.map((log, index) => (
                                    <tr className="log-table-row" key={log.id || `log-${index}`}>
                                        <td>{log.eventType}</td>
